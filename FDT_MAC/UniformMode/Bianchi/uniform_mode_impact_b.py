...
###valoresNo=[10,50,100,200,300,500]
...
for a in range (len(valoresFrame1)):
	for b in range (len(valoresFrame2)):
		...
		nomeArquivo = "/home/lucas/research/tese/figuras/cap5/bianchiUniformMode/resultado802_11b_F1_" + str(valoresFrame1[a]) + "_F2_" + str(valoresFrame2[b]) + ".txt"
		...
		for i in range (len(valoresNo)):
		    ...
		    Ts_Bidirecional = RTS + SIFS + DELTA + CTS + SIFS + DELTA + 0.5*(CTS + SIFS + DELTA) + HEADERS + T_FRAME + SIFS + DELTA + ACK + DIFS + DELTA
		    Ts_Unidirecional = RTS + SIFS + DELTA + CTS + SIFS + DELTA + 0.5*(CTS + SIFS + DELTA) + CTS + SIFS + DELTA + HEADERS + T_FRAME + SIFS + DELTA + ACK + DIFS + DELTA
		    Tc = RTS + DIFS + DELTA

		    RTS        = valoresLog1[a] + 5 + 5
		    CTS        = valoresLog1[a] + 5 + 5

		    Ts_Bidirecional_Min = RTS + SIFS + DELTA + CTS + SIFS + DELTA + 0.5*(CTS + SIFS + DELTA) + HEADERS + T_FRAME_Min + SIFS + DELTA + ACK + DIFS + DELTA
		    Ts_Unidirecional_Min = RTS + SIFS + DELTA + CTS + SIFS + DELTA + 0.5*(CTS + SIFS + DELTA) + CTS + SIFS + DELTA + HEADERS + T_FRAME_Min + SIFS + DELTA + ACK + DIFS + DELTA
